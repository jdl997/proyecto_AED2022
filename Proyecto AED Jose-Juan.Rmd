...
# Introducción
El 11 de marzo de 2020, la Organización Mundial de la Salud declaró una pandemia. Rápidamente se paralizó el mundo. El 11 de marzo de 2021, se confirmaron 117.573.007 casos y se registraron 2.610.925 muertes ([OMS](https://www.frontiersin.org/articles/10.3389/fspor.2021.695922/full#B36)). Los países afectados aplicaron diversos niveles de respuesta a lo largo del año, incluyendo restricciones sociales y cierres laborales, que impactaron de manera importante a las sociedades, más allá de los daños sanitarios directamente atribuidos al COVID-19. En vista de las masas que mueve el deporte, especialmente el deporte de élite, este tipo de eventos se debió suspender. Eventos deportivos como los Juegos Olímpicos y Paralímpicos de Tokio fueron aplazados. Gracias a la evolución en los tratamientos y a la aplicación de estrictos protocolos sanitarios, algunos campeonatos profesionales se reanudaron con algunas restricciones cómo la de efectuarse a puerta cerrada en estadios vacíos. Bajo estas nuevas circunstancias, surge la inquietud al respecto de **cómo esta situación extraordinaria afectó el rendimiento de los equipos de fútbol de las ligas europeas**, en vista de la supuesta ventaja que ofrecían los estadio con la mayoría de espectadores alentando al equipo local. Según estudios ^1^ la ventaja de jugar de local se soporta en datos que demuestran de manera consistente que los equipos locales ganan más del 50% de los partidos jugados con un calendario equilibrado de local y visitante.
...
<font face="Courier New" size=2 align="left" >
...
### Temporadas 2021-2022 {.tabset .tabset-fade .tabset-pills}
...
En vista de que concluimos que las columnas no comunes a todas las fuentes no son relevantes para el proyecto, las desestimamos, lo cuál representa una nueva base de datos de información de los partidos disputados en las tres temporadas (2018-2019, 2019-2020 y 2020-2021) de la Superliga de Grecia, cuyas dimensiones son `r ncol(base18_21)`columnas y `r nrow(base18_21)` registros, caracterizada por grupos de variables que se refieren a:

* Datos básicos del partido
* Estadísticas del juego
* Cuotas de casas de apuestas

Adicionalmente, a partir de la investigación que hemos realizado respecto a la Superliga de Grecia, y en consecuencia del objetivo que persigue el presente proyecto de analítica, determinamos que las siguientes `r ncol(base_filtrada)` columnas son de interés para el presente análisis:
...
rango_categorias = c( paste(format(min(base_filtrada$FECHA),"%d-%m-%Y"),format(max(base_filtrada$FECHA),"%d-%m-%Y"),sep = " a "), collapse(unique(base_filtrada$EQUIPO_LOCAL), sep = ", "), collapse(unique(base_filtrada$EQUIPO_VISITANTE),sep = ", "),
...
knitr::kable(tabla_variables, "simple",   col.names = c('VARIABLE', 'TIPO DE VARIABLE', 'Rango o Categorías'), align = "ll", caption = "Tabla. Descripción de las Variables para el análisis")

```

Una muestra de los datos se puede observar a continuación:

```{r}
head(base_filtrada[, 1:12])
```
...
tabla_freq(base_filtrada$FECHA)
...
tabla_freq(base_filtrada$EQUIPO_LOCAL)
...
tabla_freq(base_filtrada$EQUIPO_VISITANTE)
```

Resultado al finalizar el partido
```{r}
base_filtrada$RESULTADO_FINAL = factor(base_filtrada$RESULTADO_FINAL)
tabla_freq(base_filtrada$RESULTADO_FINAL)
```

Resultado al finalizar en el primer tiempo
```{r}
base_filtrada$RESULTADO_MITAD = factor(base_filtrada$RESULTADO_MITAD)
tabla_freq(base_filtrada$RESULTADO_MITAD)

```
Casteada de variables cuantitativa discreta(integer) y continua (numeric)
```{r}
...
base_filtrada$VISITANTE_CORNER      = as.integer(base_filtrada$VISITANTE_CORNERS)
...
```
Se procede a identificar los registros invalidos y atipicos para ser corregidos encontrando los resultados de los partidos en la siguiente pagina web 
```{r}
##base_filtrada[base_filtrada$LOCAL_GOLES>]
```
...
```{r}
tabla_freq(base_filtrada$VISITANTE_GOLES)
```