...
Equipo Fecha
```{r}
base_filtrada$Date = as.Date(base_filtrada$Date,format= "%Y-%m-%d")
tabla_freq(base_filtrada$Date)
...
tabla_freq(base_filtrada$HomeTeam)
...
tabla_freq(base_filtrada$AwayTeam)
```

Resultado al finalizar el partido
```{r}
base_filtrada$FTR = factor(base_filtrada$FTR)
tabla_freq(base_filtrada$FTR)
```

Resultado al finalizar en el primer tiempo
```{r}
base_filtrada$HTR = factor(base_filtrada$HTR)
tabla_freq(base_filtrada$HTR)

```
Casteada de variables cuantitativa discreta(integer) y continua (numeric)
```{r}
base_filtrada$FTHG = as.integer(base_filtrada$FTHG)	
base_filtrada$FTAG = as.integer(base_filtrada$FTAG)		
base_filtrada$HTHG = as.integer(base_filtrada$HTHG)	
base_filtrada$HTAG = as.integer(base_filtrada$HTAG)		
base_filtrada$HS = as.integer(base_filtrada$HS)	
base_filtrada$AS = as.integer(base_filtrada$AS)	
base_filtrada$HST = as.integer(base_filtrada$HST)	
base_filtrada$AST = as.integer(base_filtrada$AST)	
base_filtrada$HFKC = as.integer(base_filtrada$HFKC)	
base_filtrada$AFKC = as.integer(base_filtrada$AFKC)	
base_filtrada$HC = as.integer(base_filtrada$HC)	
base_filtrada$AC = as.integer(base_filtrada$AC)	
base_filtrada$HY = as.integer(base_filtrada$HY)	
base_filtrada$AY = as.integer(base_filtrada$AY)	
base_filtrada$HR = as.integer(base_filtrada$HR)	
base_filtrada$AR = as.integer(base_filtrada$AR)	
...
```{r}
base_filtrada[base_filtrada$FTHG>]
```
