...
La presente revisión tiene como finalidad estudiar acerca de **cómo esta situación de pandemia afectó el rendimiento de los equipos de fútbol durante sus juegos de local, particularmente en la Súperliga de fútbol de Grecia**, considerando el hecho de que los partidos se jugaron a puerta cerrada.

Si bien nos ocuparemos en un futuro de revisar además un modelo estadístico para medir el efecto de jugar a puerta cerrada, durante situación de pandemia, sobre la ventaja de jugar de local, **en esta   etapa cubrimos lo estrictamente concerniente al *análisis exploratorio de los datos* inputados al respecto de nuestra pregunta de investigación**.
...
Se carga el archivo "Grecia.xlsx" el cual contiene el dataset de los partidos jugados en grecia.
Se busca identificar: ¿que variables estan disponibles y que tipo de variables son?
```{r}
base=read_excel("Grecia.xlsx")
str(base)
```
...
```{r}
base$HomeTeam = factor(base$HomeTeam)
base$AwayTeam = factor(base$AwayTeam)
base$FTR = factor(base$FTR)
base$HTR = factor(base$HTR)
```
