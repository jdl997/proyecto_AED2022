...
```{r Packages and functions, echo=TRUE, message=FALSE, warning=FALSE}
library(readxl)
library(reshape2)
library(dplyr)
library(e1071)
...
```{r echo=FALSE}
base_etnias = base[base$ESTU_TIENE_ETNIA == "SI" & !is.na(base$ESTU_TIENE_ETNIA), c("ESTU_TIENE_ETNIA", "ESTU_ETNIA")]

base_etnias = base_etnias[order(base_etnias$ESTU_TIENE_ETNIA, base_etnias$ESTU_ETNIA), ]
tabla_freq( base_etnias$ESTU_ETNIA)
```
> Distribución de Etnias de la categorías con etnia SI

Observamos que los 3.276 registros clasificados con etnia SI, 3.010 contienen su respectivo registro étnico y 266 no lo tienen (NA).  Esta es una anomalía que debe corregirse antes de emprender el análisis.


```{r echo=FALSE}
base_etnias = base[base$ESTU_TIENE_ETNIA == "NO" & !is.na(base$ESTU_TIENE_ETNIA), c("ESTU_TIENE_ETNIA", "ESTU_ETNIA")]
tabla_freq( base_etnias$ESTU_ETNIA)
```
> Distribución de categorías con etnia NO

Observamos que los 45.098 registros clasificados como que NO tienen etnia, 76 contienen registro étnico, lo cuál es una inconsistencia que debería ser corregida clasificándolos como que SI tiene etnia.

```{r echo=FALSE}
base_etnias_corregidas = base[!is.na(base$ESTU_ETNIA), c("ESTU_TIENE_ETNIA", "ESTU_ETNIA")]
tabla_freq( base_etnias_corregidas$ESTU_ETNIA)
```

> Distribución de categorías con etnia SI, a partir del análisis anterior

En total serán 3.086 resgistros clasificados como que SI tienen etnia y coinciden con su respectivo registro de clasificación étnica.
...
inspect_na(base)
```
Realizamos la validación de _missing values_ sobre de los campos trabajados hasta el momento

```{r echo=FALSE}
base_completa = base[ , c("MOD_INGLES_DESEM","ESTU_FECHANACIMIENTO", "ESTU_TIENE_ETNIA", "ESTU_ETNIA", "ESTU_LIMITA_CONDICIONESPECIAL","ESTU_LIMITA_SORDO","ESTU_LIMITA_INVIDENTE","ESTU_LIMITA_MOTRIZ", "ESTU_LIMITA_AUTISMO")] 
capture.output(aggr(base_completa, numbers=TRUE, prop=FALSE, sortVars=TRUE, cex.axis = 0.6, cex.numbers = 0.6, labels=c("D_INGLES","F_NCMTO", "T_ETNIA", "ETNIA", "L_CNDESP","L_SORDO","L_INVID","L_MOTRIZ", "L_AUTISM")), file=tempfile())
...
1.- Corregimos registros que indican NO tiene etnia y tienen registro válido en etnia.  Ahora quedan con SI tiene etnia.
2.- Corregimos registros que indican SI tiene etnia, sin embargo tienen NA en etnia.  Ahora quedan con NA en si tienen etnia.
...
### Corrección "TIENE ETNIA" SI con registro de ETNIA NA --->  "TIENE ETNIA" NA
base$ESTU_TIENE_ETNIA[base$ESTU_TIENE_ETNIA=="SI" & is.na(base$ESTU_ETNIA)] = NA
tabla_freq(base$ESTU_TIENE_ETNIA)
```
