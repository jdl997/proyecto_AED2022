...
Variable para saber si se jugo con publico por la pandemia
```{r}
base_filtrada$Con_Publico = ifelse(base_filtrada$FECHA >= as.Date("2020-03-03") & base_filtrada$FECHA < as.Date("2021-07-09"), "NO", "SI")
```

Se decide crear la variable sintetica "Es_equipo_grande" el cual es un indicador de si un equipo es de renombre. Esta variable se calcula a partir de la cantidad de titulos obtenidos. [Estadisticas](https://es.wikipedia.org/wiki/Superliga_de_Grecia#T%C3%ADtulos_por_club)
![](titulos.png)
```{r}
