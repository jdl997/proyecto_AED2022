...
```{r}
capture.output(aggr(base_filtrada, numbers=TRUE, prop=FALSE, sortVars=TRUE, cex.axis = 0.5, cex.numbers = 0.5), file=tempfile())
columnas_na = inspect_na(base_filtrada) 
pct_na = 66
columnas_na = columnas_na[columnas_na$pcnt > pct_na,]

```
Al realizar la validación de campos vacios, como ilustra el anterior gráfico, 478 de los registros se concentran en `r length(columnas_na$col_name)` variables [*`r gsub(",", ", ", collapse(sort(columnas_na$col_name)))`*] con valores NA, lo cual representa más del 66% del total de los datos. En vista de que se trata de información de menor relevancia para el análisis del proyecto (Estadísticas de juego y cuotas de casas de apuestas) nuestra decisión es prescindir de estas columnas.
...
```
Las nuevas dimensiones de la base de datos, con este cambio son `r ncol(base_filtrada)` columnas y `r nrow(base_filtrada)` registros.
...
```
Adicionalmente, al realizar el análisis de las variables restantes con valores nulos [*`r gsub(",", ", ", collapse(sort(columnas_na$col_name)))`*] encontramos que estas se distribuyen en 11 registros. Uno de los registros  sólo dispone información de fecha, con lo cual no representa una obsevación válida para el análisis y procedemos a borrarla. En cuanto a las demás pérdidas, revisamos con el equipo de recolección la posibilidad de imputarlos por medio de *Web Scraping* manual, siendo efectivamente identificada la información en la fuente del siguiente enlace en [transfermarkt.mx](https://www.transfermarkt.mx/).
```{r}
...
base_filtrada$RESULTADO_MITAD[base_filtrada$FECHA == as.Date("2019-08-31") & base_filtrada$EQUIPO_LOCAL== "Panathinaikos"]= "Visitante"
...
## Giannina	VS Larisa
base_filtrada$LOCAL_GOLES[base_filtrada$FECHA == as.Date("2020-11-04") & base_filtrada$EQUIPO_LOCAL== "Giannina"]= 1
base_filtrada$VISITANTE_GOLES[base_filtrada$FECHA == as.Date("2020-11-04") & base_filtrada$EQUIPO_LOCAL== "Giannina"]= 2
```

```{r}
base_filtrada[base_filtrada$FECHA == as.Date("2018-09-16") | (base_filtrada$FECHA == as.Date("2018-09-15") & base_filtrada$EQUIPO_LOCAL== "OFI Crete") | (base_filtrada$FECHA == as.Date("2018-09-17") & base_filtrada$EQUIPO_LOCAL== "Aris") | (base_filtrada$FECHA == as.Date("2018-10-28") & base_filtrada$EQUIPO_LOCAL== "Panionios") | (base_filtrada$FECHA == as.Date("2019-03-17") & base_filtrada$EQUIPO_LOCAL== "Panathinaikos") |(base_filtrada$FECHA == as.Date("2019-08-31") & base_filtrada$EQUIPO_LOCAL== "Panathinaikos") | (base_filtrada$FECHA == as.Date("2020-11-04") & base_filtrada$EQUIPO_LOCAL== "Giannina") ,]
```
...
```
Finalizada la solución de estas anomalías, las nuevas dimensiones de la base de datos son `r ncol(base_filtrada)` columnas y `r nrow(base_filtrada)` registros.
...
## Valores inválidos o sin referencia en el diccionario de variables

La unica anomalia de valores inválidos que encontramos, fue corregida al encontrar valores de goles negativos lo cual no tiene evidentemente sentido en el contexto.

## Valores inconsistentes

Se trata de buscar valores inconsistentes al comprobar que:
1) El marcador a mitad del tiempo siempre tiene que ser menor al resultado luego de terminar el partido
2) Los goles marcado deben ser congruentes con el resultado del partido. Ejemplo: "Si el partido concluyo 1-1 en goles el resultado_final debe ser empatado".
```{r}
base_filtrada[base_filtrada$LOCAL_GOLES_MITAD > base_filtrada$LOCAL_GOLES,]

base_filtrada[(base_filtrada$LOCAL_GOLES > base_filtrada$VISITANTE_GOLES & base_filtrada$RESULTADO_FINAL != "Local") | (base_filtrada$LOCAL_GOLES == base_filtrada$VISITANTE_GOLES & base_filtrada$RESULTADO_FINAL != "Empate") | (base_filtrada$LOCAL_GOLES < base_filtrada$VISITANTE_GOLES & base_filtrada$RESULTADO_FINAL != "Visitante"),]
...
Creamos una Variable para saber si el partido se jugó sin público por las medidas aplicadas durante la pandemia.  
```{r}
base_filtrada$Con_Publico = ifelse(base_filtrada$FECHA > as.Date("2020-07-09") & base_filtrada$FECHA < as.Date("2021-07-24"), "NO", "SI")
```

Se decide crear la variable sintética "Es_equipo_grande" la cual es un indicador de si un equipo es de renombre. Esta variable se calcula a partir de la cantidad de títulos obtenidos. [Estadisticas](https://es.wikipedia.org/wiki/Superliga_de_Grecia#T%C3%ADtulos_por_club)
![](titulos.png)

```{r}
equipos_grandes = c("Olympiakos", "Panathinaikos", "AEK", "PAOK", "Aris")
base_filtrada$Es_equipo_local_grande = ifelse(base_filtrada$EQUIPO_LOCAL %in% equipos_grandes, "SI", "NO")
str(base_filtrada)
```
...
Este informe ha sido dedicado a documentar el trabajo de los equipos de analítica para preparar los datos disponibles que permitan  evaluar cómo la situación de pandemia afectó el rendimiento de los equipos de fútbol en sus juegos de local, particularmente en dicho efecto sobre los equipos de la Súperliga de fútbol de Grecia. 

Hemos revisado estos aspectos a lo largo de cuatro secciones en donde abordamos detalles respecto a los datos recolectados y a su estructura de variables. Nos referimos además a los hallazgos en materia de anomalías y a las respectivas transformaciones  realizadas en consecuencia respecto a:
- Eliminar doce registros duplicados que introducian sesgo a futuros procesos analíticos. 
- Eliminar nueve columnas que no aportaban información suficiente debido a que contenían valores vacíos en más del 66% de los registros.
- Imputar información conseguida a través de *web scrapping* manual para completar los registros identificados con datos nulos en variables que pudieron ser recolectadas y agregar valor a futuros procesos analíticos.
- No encontramos errores de digitación después de contrastar la información disponible en la base de datos con los datos verificados en fuentes oficiales de la Superliga de Grecia.
- Imputar los datos correctos asociados a las anomalías en el caso específico de los casos de goles negativos y por fuera del rango válido de acuerdo con las estadísticas.

También hemos descrito las variables que adicionamos a la base de datos, las cuales consolidan información respecto a si el partido se desarrolló o no durante el período de ausencia de público, e indican el renombre de un equipo a partir de los títulos obtenidos, a fin de para favorecer a los trabajos futuros de este proyecto de analítica.

Algunas recomendaciones de interés para los equipos de recolección de datos consisten en asegurar la extracción de las fuentes de origen usando herramientas que automaticen el levantamiento, con calidad, y reduzcan el esfuerzo de los análisis de exploración de los datos por los demás equipos de analítica.  
